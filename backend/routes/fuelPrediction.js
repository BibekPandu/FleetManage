...
const calculateFuelConsumption = (inputData) => {
  try {
    ...
    // Vehicle type impact
    fuelConsumption +=
      REGRESSION_COEFFICIENTS.vehicleType[inputData.vehicleType] || 0;

    // Engine type impact
    fuelConsumption +=
      REGRESSION_COEFFICIENTS.engineType[inputData.engineType] || 0;

    // Load impact (per 100kg)
    const loadImpact =
      (inputData.averageLoad / 100) * REGRESSION_COEFFICIENTS.loadCoefficient;
    fuelConsumption += loadImpact;

    // Speed impact (per 10 km/h above 60)
    const speedImpact =
      Math.max(0, (inputData.averageSpeed - 60) / 10) *
      REGRESSION_COEFFICIENTS.speedCoefficient;
    fuelConsumption += speedImpact;

    // Distance impact (per 100km)
    const distanceImpact =
      (inputData.distance / 100) * REGRESSION_COEFFICIENTS.distanceCoefficient;
    fuelConsumption += distanceImpact;

    // Age impact
    const ageImpact =
      inputData.vehicleAge * REGRESSION_COEFFICIENTS.ageCoefficient;
    ...
    const fallbackPrice =
      NEPAL_FUEL_PRICES_NPR[inputData.engineType] ??
      NEPAL_FUEL_PRICES_NPR.diesel;
    const fuelPriceNpr =
      typeof inputData.fuelPriceNPR === "number" &&
      !Number.isNaN(inputData.fuelPriceNPR)
    ...
    return {
      ...
      efficiency:
        Math.round((inputData.distance / totalFuelNeeded) * 100) / 100, // km/l
    };
  } catch (error) {
...
router.post(
  ...
  [
    ...
    body("averageLoad")
      ...
      .withMessage("Average load must be between 0 and 5000 kg"),
    body("averageSpeed")
      .isFloat({ min: 10, max: 120 })
      .withMessage("Average speed must be between 10 and 120 km/h"),
    body("distance")
      .isFloat({ min: 1, max: 1000 })
      .withMessage("Distance must be between 1 and 1000 km"),
    body("vehicleAge")
      .isFloat({ min: 0, max: 20 })
      .withMessage("Vehicle age must be between 0 and 20 years"),
    body("weather")
      .isIn(["sunny", "rainy", "snowy", "windy"])
      .withMessage("Valid weather condition is required"),
    body("terrain")
      .isIn(["flat", "hilly", "mountainous", "urban"])
      .withMessage("Valid terrain type is required"),
    body("fuelPriceNPR")
      .optional()
      .isFloat({ min: 0 })
      .withMessage("Fuel price must be a positive number"),
  ],
  async (req, res) => {
    try {
      ...
      try {
        ...
        console.warn(
          " Failed to persist fuel prediction history:",
          dbErr.message
        );
      }
...
router.post(
  ...
  [
    ...
    body("cylinders")
      .isInt({ min: 3, max: 12 })
      .withMessage("cylinders must be 3-12"),
    body("horsepower").isFloat({ gt: 0 }).withMessage("horsepower must be > 0"),
    body("displacement_in_cc")
      .isFloat({ gt: 0 })
      .withMessage("displacement_in_cc must be > 0"),
    body("weight_in_kg")
      .isFloat({ gt: 0 })
      .withMessage("weight_in_kg must be > 0"),
    body("acceleration")
      .isFloat({ gt: 0 })
    ...
    body("origin")
      .isInt({ min: 1, max: 3 })
      .withMessage("origin must be 1,2,3"),
    body("distance")
      .optional()
      .isFloat({ min: 1, max: 1000 })
      .withMessage("distance must be 1-1000"),
    body("fuelPriceNPR")
      .optional()
      .isFloat({ min: 0 })
      .withMessage("fuel price must be >= 0"),
  ],
  async (req, res) => {
    try {
      const errors = validationResult(req);
      ...
      const B0 =
        typeof process.env.MLR_INTERCEPT === "string"
          ? Number(process.env.MLR_INTERCEPT)
          : 1.2;
      ...
      if (!Number.isFinite(mileage_kpl_unclamped)) {
        return res
          .status(400)
          .json({
            message: "Invalid inputs produced a non-finite mileage value",
          });
      }
      ...
      const totalFuelNeeded =
        useDistance > 0 ? (fuelConsumptionPer100km * useDistance) / 100 : 0;
      ...
      const fallbackPrice =
        typeof NEPAL_FUEL_PRICES_NPR.diesel === "number"
          ? NEPAL_FUEL_PRICES_NPR.diesel
          : 175;
      const price =
      ...
      const prediction = {
        fuelConsumptionPer100km:
          Math.round(fuelConsumptionPer100km * 100) / 100,
        totalFuelNeeded: Math.round(totalFuelNeeded * 100) / 100,
        totalCost: Math.round(totalCostNpr * 100) / 100,
      ...
      try {
        ...
        console.warn(
          " Failed to persist fuel prediction history (kpl):",
          dbErr.message
        );
      }
      ...
      return res
        .status(500)
        .json({ message: "Error calculating fuel prediction (kpl)" });
    }
  }
);
...
router.get("/analytics", authenticateToken, async (req, res) => {
  try {
    ...
    const [totalResult] = await pool.execute(
      "SELECT COUNT(*) as total FROM fuel_predictions"
    );
...
router.post("/reset-history", authenticateToken, async (req, res) => {
  try {
